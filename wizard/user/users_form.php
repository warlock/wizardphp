		...
		if ($class == "new") {
			if (isset($_GET["verify"])) {
				...
				if (!$dbs->EOF) {
					...
					print "<b>".t('users','verified').":</b> ".$dbs->fields[mail]."<br>";
		...
		if (isset($_COOKIE["sToken"])) {
			...
			$dbs = $db->Execute($select_session);
				if (!$dbs->EOF) {
					...
					<?
					print t('users','logged_with')." ".$userlogs->fields[mail];
					?> <b><a href="#" onclick="logoutform.submit()"><? t('users','logout'); ?></a></b><br><?
				} else {
					...
					<input type="hidden" name="form_id" value="users">
					<? t("users","mail"); ?> <input type="text" name="mail" />   <br>
					<? t("users","password"); ?> <input type="password" name="password" /> <br>
					<input type="submit" value="<? t("users","login"); ?>" /> 
					</form> 
					<?
				}
			$db->Close();
