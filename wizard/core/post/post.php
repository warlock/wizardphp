...
if (count($_POST) > 1) {
	if ($_POST[action] == "controller") {
		$file_wizard_post_controller = "wizard/controller/".$wizard_post[postcontroller]."/post.php";
		if (file_exists($file_wizard_post_controller)) {
			include($file_wizard_post_controller);
		} else {
			wizard_error("Controller <b>".$wizard_post[postcontroller]."</b> no contains \"post.php\" file.<br>");
		}
	} else {
		//CRUD
